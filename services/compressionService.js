...
const compressFiles = async (files, req) => {
    ...
    try {
        ...
        if (fs.existsSync(zipFileName)) {
            ...
            fs.readdir(uploadPath, function (err, oldFiles) {
                ...
                oldFiles.forEach(file => {
                    if (!file.endsWith('_original')) {
                        let absolutePath = path.join(process.env.UPLOAD_PATH, req.session.id, file);
                        zip.addLocalFile(absolutePath);
                    }
                });
                // Now add new files also to .zip
                files.forEach(file => {
                    let absolutePath = path.join(process.env.UPLOAD_PATH, req.session.id, file.name);
                    zip.addLocalFile(absolutePath);
                });
            ...
            const singleFilename = path.join(process.env.UPLOAD_PATH, req.session.id, req.session.filename);
